...
class STKAgent:
    ...
    def __init__(self, graphicConfig: pystk.GraphicsConfig, raceConfig: pystk.RaceConfig, id=1):
        ...
        self.config.screen_width = 600
        self.config.screen_height = 400
        ...
        raceConfig = pystk.RaceConfig(num_kart=1, laps=1, track='hacienda')
        raceConfig.players[0].controller = pystk.PlayerConfig.Controller.PLAYER_CONTROL
        ...
        self.AI = False # was true
        #print(raceConfig.num_kart)
    ...
    def _point_on_track(self, distance, track, offset=0.0):
        ...
        #print(self)
        path_dist = self.path_distance
        nodes = self.path_nodes
        node_idx = np.searchsorted(path_dist[..., 1],
                                   distance % path_dist[-1, 1]) % len(nodes)
        d = path_dist[node_idx]
        x = self.path_nodes[node_idx]
        t = (distance + offset - d[0]) / (d[1] - d[0])
        points = x.points
        ans = points[1] * t + points[0] * (1 - t)
        return ans
    ...
    def _update_action(self, action: list):
        ...
        if (hasattr(action, 'acceleration')):

          self.currentAction.acceleration = action.acceleration
          self.currentAction.brake = action.brake
          self.currentAction.steer = action.steer
          self.currentAction.fire = False
          self.currentAction.drift = action.drift
          self.currentAction.nitro = action.nitro
    ...
    def step(self, action=None):
        if self.AI:
            ...
            #player = self.race.config.players[0]
            kart = self.playerKart
            track = self.track
            #print(self, " step \n")
            #print(vars(self))
            #kart = self.playerKart
